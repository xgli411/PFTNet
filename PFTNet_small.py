...
class DeepRFT_Small(nn.Module):
    def __init__(self, num_res=4, inference=False):
        ...
        self.encoder_level3 = nn.Sequential(*[
            TransformerBlock(dim=int(base_channel * 2 ** 2), ffn_expansion_factor=self.ffn_expansion_factor,
                             bias=False) for i in range(self.num_blocks[2])])
        ...
        self.decoder_level2 = nn.Sequential(*[
            TransformerBlock(dim=int(base_channel * 2 ** 1), ffn_expansion_factor=self.ffn_expansion_factor,
                             bias=False, att=True) for i in range(self.num_blocks[1])])
    ...
    def forward(self, x):
        ...
        if not self.inference:
            ...
            return z + x
class DeepRFT_Small_flops(nn.Module):
    def __init__(self, num_res=4, inference=True):
        super(DeepRFT_Small_flops, self).__init__()
